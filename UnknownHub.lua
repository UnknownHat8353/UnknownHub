...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>UnknownHat8353 Hub Hello<Color=/>"):Display()
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Join discord for latest updates<Color=/>"):Display()
    wait(.29)
end)
...
OrionLib:MakeNotification({
    Name = "UnknownHat8353 Hub",
    Content = "Wait for Hub to load config before using",
    Image = "rbxassetid://10790918487",
    Time = 10
})
...
IQ:AddToggle({
    ...
    Callback = function(Value)
        ...
        IQ:AddLabel("For those who don't understand, this is automatic dummy beating.")
    ...
    while wait(.1) do
        if LockLevel then
            if game.Players.localPlayer.Data.Level.Value >= LockLevelValue then
                game.Players.localPlayer:Kick("Lock Level : Done[My level is so good!]")
            end
        end
    end
end)
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "UnknownHat8353 Hub(Noitification)",
